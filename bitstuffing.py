...
def handle_client(client_socket):
    while True:
        ...
        # stuff input string
        stuffed_str = bit_stuffing(input_str)
...
def start_client():
    ...
    while True:
        ...
        input_str = input('Enter a binary input string (or "quit" to exit): ')
        if input_str == 'quit':
            break
        ...
        output_str = ""
        count = 0
        for i in range(len(stuffed_str)):
            if stuffed_str[i] == "1":
                count += 1
            else:
                count = 0
            output_str += stuffed_str[i]
            if count == 5 and i < len(stuffed_str) - 1 and stuffed_str[i+1] == "0":
                i += 1
                count = 0
